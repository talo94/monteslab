    ...
    <div>
      {showLegend && (
        <div className="mb-5 flex flex-wrap gap-x-4 gap-y-1 text-xs text-[#7b6d60]">
          <span>
            <strong className="font-medium text-[#382e22]">Negrita</strong> —
            coro
          </span>
          <span>
            <span className="font-medium text-[#b54a32]">Rojo</span> —
            indicación musical
          </span>
        </div>
      )}
...
function formatToken(part: string, index: number): ReactNode {
  ...
  if (part.startsWith("[[") && part.endsWith("]]")) {
    return (
      <span key={index} className="text-[13px] font-normal text-[#b54a32]">
        {part.slice(2, -2)}
      </span>
    )
  }
  ...
  if (part.startsWith("/") && part.endsWith("/")) {
    ...
    return (
      <span key={index}>/{formatInlineEmphasis(inner, String(index))}/</span>
    )
  }
...
function formatInlineEmphasis(text: string, keyPrefix: string): ReactNode {
  ...
  return parts.map((piece, pieceIndex) => {
    ...
    if (piece.startsWith("**") && piece.endsWith("**")) {
      return (
        <strong key={key} className="font-medium text-[#382e22]">
          {piece.slice(2, -2)}
        </strong>
      )
    }
